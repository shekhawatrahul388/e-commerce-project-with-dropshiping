  ...
  const loadStore = async () => {
    ...
    try {
      const [storeResponse, productsResponse] = await Promise.all([
        api.get(`/store/${encodeURIComponent(storeSlug)}`),
        api.get(`/store/${encodeURIComponent(storeSlug)}/products`),
      ]);
      if (!active) return;
      ...
      if (active) {
        setError(requestError?.response?.status === 404 ? "Store not found" : "Unable to load this store");
      }
    } finally {
